...
| E23 | [docs, *Services › Get availability*] `UsableResources` is "resources that are usable (i.e., not out of order)", and `Occupied` is "bookings assigned (reservations and blocks)". The docs define **no** "sellable" formula. They give `UsableResources − Occupied` as free capacity, and point to the sell-limit endpoint for exact inventory.                                                     | Out-of-order rooms are already out of `UsableResources`: subtracting `OutOfOrderBlocks` again double-counts them. `reservations/add` (with `CheckOverbooking`) has the final word. |
| E24 | **`reservations/add` with several reservations is all-or-nothing** on an availability refusal. Two identical stays for a one-room category in one call answered `403` "no availability", and neither was created: a control query with the same filter found the earlier reservations of that guest and nothing new. [observed]                                                                     | Several rooms are booked in **one** call, which cannot leave a half-made booking. The rooms share one `GroupId`.                                                                   |
| E25 | `reservations/getAll/2023-06-06` takes a `ScheduledEndUtc` interval and pages with a cursor; an interval of 100 days answered **400 "The interval must not exceed 3M1D."**, 90 days answered. A past stay reads `State: "Processed"` with the guest as `AccountId`. [observed 2026-09-24] | `list_reservations` takes a `checkOutWindow` of at most 90 days, refused locally before Mews is asked. |
| E26 | `customers/getAll` by `CustomerIds` returns `FirstName`, `LastName`, `Email` and `Phone`; the demo's guests carry no phone. [observed 2026-09-24] | The consumer's CRM identity is the phone: a demo backfill imports no one, a real property's does. |
| E27 | `orderItems/getAll` by `ServiceOrderIds` (the reservation ids) returns each charge with `ServiceOrderId`, `Amount { Currency, GrossValue }`, `Type` (e.g. `NightRebate` as a negative) and `AccountingState`. [observed 2026-09-24] | `list_order_items`: what a stay cost, for the consumer to sum. |
| E28 | Room types carry no image ids in this API version; photos are assigned through `resourceCategoryImageAssignments/getAll`, which answered 200 with an empty list on the demo. `images/getUrls` turned the enterprise's cover and logo image ids into public `cdn.mews-demo.com` URLs that downloaded as `image/png`. [observed 2026-09-24; an assignment's own fields are Mews' documented shape, not yet observed] | `list_resource_categories` returns each room type with its `ImageUrls`; a failed photo read returns the room types without photos (#1081). |
