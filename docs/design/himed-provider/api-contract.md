...
Mensajes de token observados en doc: _"El token ingresado no es correcto"_, _"Se envía un token no
conocido"_, _"El token está vacío"_, _"No se envía el token en el JSON"_ → `PROVIDER_AUTH_EXPIRED`.
...
- **`401` está sobrecargado**: HiMed lo usa para token inválido **y** para errores de validación. El envelope de
  ...
  `himed-scheduling/errors.ts`. Un **200 también puede traer un error capturado** (`estado:"error"`), y se trata como error.
...
| Respuestas de **todos los reads** (existePaciente, list\_\*, disponibilidad)            | ✅ **verificadas en vivo** (ver §2.8)                                                                                 |
